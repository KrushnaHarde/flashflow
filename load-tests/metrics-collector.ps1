...
while (-not (Test-Path $flagPath)) {
    ...
    try {
        $res = Invoke-RestMethod -Uri "http://localhost:8080/actuator/metrics/reservations.expired.count" -TimeoutSec 1
    ...
    try {
        $res = Invoke-RestMethod -Uri "http://localhost:8080/actuator/metrics/orders.confirmed.count" -TimeoutSec 1
